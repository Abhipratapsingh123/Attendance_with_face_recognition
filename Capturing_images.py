...
existing_ids = {fname.split(".")[1] for fname in os.listdir(uploads) if fname.startswith("user.")}

if person_id in existing_ids:
    print(f"Error: ID {person_id} already exists! Please choose a different ID.")
    sys.exit(1)
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.imwrite(os.path.join(uploads, f"user.{person_id}.{image_count}.jpg"), face_image)
...
print(f"Collected {image_count} images for person ID {person_id}. Data saved in users.csv.")
